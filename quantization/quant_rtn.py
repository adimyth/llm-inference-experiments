...
Q4_0 is llama.cpp's plain round-to-nearest quant type: each block of weights
gets a single scale, and every weight rounds to the nearest representable
value on that block's grid. No calibration data, no importance weighting.
This is the naive baseline every other method in this folder is measured
against.

llama-quantize does the actual work; this wrapper exists so the conversion is
a checked-in, reproducible step rather than a one-off shell command, and so
it reports the size reduction the essay quotes.
"""
