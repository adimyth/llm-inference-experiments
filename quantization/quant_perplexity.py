...
Lower is better: this measures how well the model predicts real held-out
text, averaged over every token in the passage. It's the primary quality
signal for comparing a quantized checkpoint against the fp16 baseline -
run identically against both, so a difference is attributable to the
quantization method and not to the eval itself.

llama-perplexity does the actual computation; this wraps it, parses its
"Final estimate: PPL = X +/- Y" line, and records the result.
"""
