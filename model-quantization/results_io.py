...
Each script (perplexity, throughput, MMLU) only knows about its own metric. This merges by checkpoint label so results/results.json ends up as one row per checkpoint with every metric filled in as each script runs, in whatever order they're run.
"""
