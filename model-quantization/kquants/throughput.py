...
Reports two numbers: pp (prompt processing, how fast the model chews through input context) and tg (text generation, how fast it produces new tokens one at a time). tg is the more telling number for quantization, since decode is memory-bandwidth bound and a smaller checkpoint moves less data per token.

llama-bench already handles warmup and repeats; this just runs it with JSON output and records the medians.
"""
