...
Unlike RTN's Q4_0 (one scale per 32-weight block, uniform treatment), the k-quant types vary bits per tensor by how much each tensor's precision tends to matter (attention vs FFN, early vs late layers), based on llama.cpp's built-in heuristics rather than a calibration pass. No importance matrix (--imatrix) is used here - that's a further refinement using calibration text to weight the heuristic, out of scope for this comparison but worth a note in the essay as the natural next lever.

Reuses the same f16 GGUF that RTN quantized from - built once, quantized from three times.
"""
