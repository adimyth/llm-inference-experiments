...
Same pp/tg split as quant_gguf_throughput.py and quant_mlx_throughput.py: prompt processing (chewing through input context) timed separately from generation (producing new tokens one at a time), so the numbers are comparable across formats.

Every timed region calls `torch.mps.synchronize()` before stopping the clock, per this repo's method notes - MPS queues work asynchronously, so skipping this times how fast Python queued the work, not how long it took.
"""
