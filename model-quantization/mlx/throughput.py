...
Mirrors quant_gguf_throughput.py's pp/tg split so the numbers are comparable across formats: prompt processing (how fast the model chews through input context) and generation (how fast it produces new tokens one at a time).

mlx_lm.stream_generate reports both directly per GenerationResponse (the final one carries the run's aggregate prompt_tps/generation_tps), so this just repeats it with a warmup pass and takes the median, matching llama-bench's own warmup+repeat discipline.
"""
